...
type ThemeLoader = (id: string) => Promise<DesktopTheme | undefined>
type ApplyTheme = (theme: DesktopTheme, themeId: string, mode: "light" | "dark", scheme: ColorScheme) => void

function isValidThemeId(store: ThemeStore, id: string) {
  return id === "oc-2" || knownThemes().has(id) || Boolean(store.themes[id])
}

function createThemeLoader(store: ThemeStore, setStore: SetStoreFunction<ThemeStore>): ThemeLoader {
...
function createApplyTheme(props: ThemeProps): ApplyTheme {
  return (theme, themeId, mode, scheme) => {
    applyThemeCss(theme, themeId, mode)
    props.onThemeApplied?.(theme, mode, scheme)
  }
}

function createIds(store: ThemeStore) {
  return () => {
...
function createOnStorage(store: ThemeStore, setStore: SetStoreFunction<ThemeStore>, load: ThemeLoader) {
  return (e: StorageEvent) => {
    if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
      const next = normalize(e.newValue)
      if (!next || !isValidThemeId(store, next)) return
...
function setupThemeMount(
  store: ThemeStore,
  setStore: SetStoreFunction<ThemeStore>,
  load: ThemeLoader,
...
function createSetTheme(store: ThemeStore, setStore: SetStoreFunction<ThemeStore>, load: ThemeLoader) {
  return (id: string) => {
    const next = normalize(id)
    if (!next || !isValidThemeId(store, next)) {
...
function createSetColorScheme(setStore: SetStoreFunction<ThemeStore>) {
...
function createPreviewTheme(store: ThemeStore, setStore: SetStoreFunction<ThemeStore>, load: ThemeLoader, applyTheme: ApplyTheme) {
  return (id: string) => {
    const next = normalize(id)
    if (!next || !isValidThemeId(store, next)) return
...
function createPreviewColorScheme(
  store: ThemeStore,
  setStore: SetStoreFunction<ThemeStore>,
  load: ThemeLoader,
...
function createCommitPreview(
  ...
  return () => {
    if (store.previewThemeId) setTheme(store.previewThemeId)
...
function createCancelPreview(store: ThemeStore, setStore: SetStoreFunction<ThemeStore>, load: ThemeLoader, applyTheme: ApplyTheme) {
  return () => {
    setStore("previewThemeId", null)
    setStore("previewScheme", null)
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: ThemeProps) => {
    ...
    const applyTheme = createApplyTheme(props)
    const ids = createIds(store)
    const loadThemes = () => Promise.all(themeIDs().map(load)).then(() => store.themes)
    const onStorage = createOnStorage(store, setStore, load)
    const setTheme = createSetTheme(store, setStore, load)
    const setColorScheme = createSetColorScheme(setStore)
    const previewTheme = createPreviewTheme(store, setStore, load, applyTheme)
    const previewColorScheme = createPreviewColorScheme(store, setStore, load, applyTheme)
    const commitPreview = createCommitPreview(store, setStore, setTheme, setColorScheme)
    const cancelPreview = createCancelPreview(store, setStore, load, applyTheme)

    onMount(() => setupThemeMount(store, setStore, load, onStorage, props))
